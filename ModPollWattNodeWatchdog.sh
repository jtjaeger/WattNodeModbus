...
FILE="/tmp/ModPollWattNode_Watchdog"
LOGFILE="/var/log/ModPollWattNode_Watchdog.log"
PROCESS="python /usr/local/bin/ModPollWattNode.py"
TimeNow=$(date +%s)
...
then
   ...
   if [ "$TimeDifference" -gt "60" ] && [ "$TimeInWatchdogFile" != "" ];
   then
